...
class ResNetShard2(ResNetBase):
    ...
    def forward(self, x_rref):
        ...
        with self._lock:
            out =  self.seq(x)
            print("process worker 2")
        return out.cpu()

class ResNetShard3(ResNetBase):
    """
    The third part of ResNet.
    """
    def __init__(self, device, *args, **kwargs):
        super(ResNetShard3, self).__init__(
    ...
    def forward(self, x_rref):
        ...
        with self._lock:
            out = self.seq(x)
            print("process worker 3")

        return out.cpu()


class ResNetShard4(ResNetBase):
    """
    The forth part of ResNet.
    """
    def __init__(self, device, *args, **kwargs):
        super(ResNetShard4, self).__init__(
            Bottleneck, 512, num_classes=num_classes, *args, **kwargs)
...
def run_master(split_size):
    ...
    model = DistResNet50(split_size, ["worker1", "worker2", "worker3", "worker4"])
    loss_fn = nn.MSELoss()
